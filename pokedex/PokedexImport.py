...
class PokedexImport:
    ...
    def importPokemon(self, pokemonData):
        types   = self.importTypes(type_data)
        weather = self.importWeather(weather_data)
...
importer.importPokemon(pokemon_data)

bulbasaur = Pokemon.objects(name="Bulbasaur")

pprint(bulbasaur[0].type.name)

if bulbasaur[0].type2 is not None:
    pprint(bulbasaur[0].type2.name)
